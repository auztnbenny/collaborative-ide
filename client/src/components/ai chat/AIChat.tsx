    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (question.toLowerCase().includes('analyze code')) {
                ...
                const aiAnswer: Message = { sender: 'ai', content: analysisResult };
                setMessages((prev) => [...prev, aiAnswer]);
            } else {
                const response = await axios.post('http://localhost:3000/api/ai/ask', { question });

                if (response.status === 200) {
                    const aiAnswer: Message = { sender: 'ai', content: response.data.answer };
                    setMessages((prev) => [...prev, aiAnswer]);
                } else {
                    toast.error(response.data.error || 'Failed to get a response from AI.');
                }
            }
        } catch (error: any) {
